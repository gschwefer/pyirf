...
reco_energy_bins = [0.1, 1, 10, 100] * u.TeV
...
def test_effective_area_table_2d():
    ...
    aeff_gammapy = create_effective_area_table_2d(
        aeff, true_energy_bins, fov_offset_bins
    )
    # test str repr works
...
def test_energy_dispersion():
    ...
    shape = (
        len(true_energy_bins) - 1,
        len(migration_bins) - 1,
        len(fov_offset_bins) - 1,
    )
    edisp = np.zeros(shape)
    edisp2d = create_energy_dispersion_2d(
...
def test_background_2d():
    from pyirf.gammapy import create_background_2d
    ...
    background2d = create_background_2d(background, reco_energy_bins, fov_offset_bins)
    str(background2d)
